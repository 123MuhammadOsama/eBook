...
const Testimonial = () => {
  ...
  const nextSlide = () => {
    setStartIndex((startIndex + 1) % reviews.length);
  };

  const prevSlide = () => {
    setStartIndex((startIndex - 1 + reviews.length) % reviews.length);
  };
  ...
  return (
    <div className='bg-black w-full h-[800px]'>
      <div className='bg-red-600 w-[600px] text-white font-bold py-8 text-center text-4xl absolute left-[650px] -rotate-6'>TESTIMONIALS</div>
      <p className='text-white pt-[150px] text-center text-2xl px-96'>Tap expert writers and a partner who gets modern content marketing. In a sea of sameness, stand out in search with E-book writing Service</p>

      <div className="flex items-center justify-center text-white pt-[50px] px-20">
        
        <div className="flex space-x-5">
          {displayedReviews.map((review) => (
            <div key={review.id} className="bg-white text-black rounded-lg p-5 shadow-md w-auto h-[400px] relative">
              <Image
              ...
              <div className="flex flex-col items-center">
                <Image src={review.imageUrl} alt="" height={200} width={200} className="w-20 h-20 rounded-full mb-3" />
        ...
        <h2 className="text-4xl font-bold mb-20">1000+ Positive Reviews Reflecting Customer Satisfaction</h2>
          ...
          <button onClick={prevSlide} className=" text-red-500 bg-white w-12  h-12 rounded-full flex items-center justify-center">
            <ChevronLeft size={100}/>
          </button>
          <button onClick={nextSlide} className="bg-white text-red-500 w-12  h-12 rounded-full flex items-center justify-center">
